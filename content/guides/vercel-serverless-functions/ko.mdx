...
<CopyBlock language="typescript" code="// app/api/hello/route.ts
...
# POST 요청
curl -X POST http://localhost:3000/api/hello -H "Content-Type: application/json" -d '{"name": "Vercel"}'
</CopyBlock>
...
<CopyBlock language="typescript" code="// app/api/users/[id]/route.ts
import { NextResponse } from 'next/server';
...
# 올바른 토큰으로 호출 - 성공
curl https://my-vercel-app.vercel.app/api/protected/data -H "Authorization: Bearer valid-token-123"
</CopyBlock>
